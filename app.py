...
# ── ATTENDANCE ────────────────────────────────────────────────────
    ...
    with st.expander("🔍 Debug info (remove after Sunday test)"):
        ...
        if not checked_in_df.empty and 'Date' in checked_in_df.columns:
            today_rows = checked_in_df[checked_in_df['Date'].astype(str) == today_str]
            st.write(f"Rows matching today ({today_str}): {len(today_rows)}")
            no_resp_today = today_rows[today_rows['Status'].astype(str).str.contains('No Response', na=False)]
            st.write(f"No Response rows today: {len(no_resp_today)}")
            if not no_resp_today.empty:
                st.write(no_resp_today[['EngineerName','EngineerEmail','Date','Status']].head(10))
        st.write(f"no_response_emails: {no_response_emails}")
        st.write(f"no_response_names: {no_response_names}")
