...
<div class="portal-header">
  <div class="portal-logo">
    <div style="width:36px;height:36px;background:white;border-radius:8px;display:flex;align-items:center;justify-content:center;margin-right:10px;font-weight:900;color:#0072BB;font-size:11px;flex-shrink:0;">SC</div>
...
# Handle URL-based navigation
params = st.query_params
if 'page' in params and params.get('page') in PAGES:
    st.session_state.page = params['page']

nav_html = '<div class="portal-nav">'
for p in PAGES:
    active_cls = "active" if st.session_state.page == p else ""
    nav_html += f'<a class="nav-item {active_cls}" href="?page={p}" style="text-decoration:none;">{p}</a>'
nav_html += '</div>'
...
with col_tl:
    new_tl = st.selectbox("👥 Viewing team:", TL_NAMES, index=TL_NAMES.index(active_tl))
    if new_tl != active_tl:
...
if not checked_in_df.empty and "EngineerName" in checked_in_df.columns:
    # Build case-insensitive lookup for my engineers
    my_eng_lower = {e.lower().strip(): e for e in my_engineers}
    for _, r in checked_in_df.iterrows():
        name_raw = str(r.get("EngineerName","")).strip()
        # Match by exact name or case-insensitive
        matched = my_eng_lower.get(name_raw.lower(), None)
        if matched:
