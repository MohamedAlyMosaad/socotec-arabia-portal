...
# CORRECT LOGIC:
# 1. Get NO-RESPONSE engineers from Daily_Exceptions (populated by the Morning Check-in flow)
# 2. Get check-in details from Daily_Attendance_Log (only real check-ins now)
# 3. For each team member: if in exceptions = no response, otherwise = checked in

today_str = str(today)  # "2026-05-07"

# Build no-response set from Daily_Exceptions (the correct source after flow fix)
...
# exc_names used for reference display only — no-response detection done above via exceptions_df
exc_names = set()
...
# ── CLAIMS ────────────────────────────────────────────────────────
# ══════════════════════════════════════════════════════════════════
# CLAIMS TAB — PRIVACY PATCH
# Drop this into app.py replacing the existing Claims section.
#
# Rules:
#   • Mohamed Mossad  → sees ALL teams' claims (full access)
#   • Any other TL    → sees ONLY their own team's claims
#   • Logging a claim → always tags it with active_tl (unchanged)
# ══════════════════════════════════════════════════════════════════

ADMIN_TL = "Mohamed Mossad"   # ← the only user who sees every team

# ── helper ────────────────────────────────────────────────────────
def filter_claims_for_user(claims: list, viewer_tl: str) -> list:
    """Return claims visible to this TL based on privacy rules."""
    if viewer_tl == ADMIN_TL:
        return claims                               # full access
    return [c for c in claims if c.get("team_leader", "") == viewer_tl]


# ══════════════════════════════════════════════════════════════════
# PAGE: CLAIMS   (replace the existing `if st.session_state.page == "Claims":` block)
# ══════════════════════════════════════════════════════════════════
if st.session_state.page == "Claims":
    ...
    # Privacy notice for non-admin TLs
    if not is_admin:
        st.info(f"🔒 **Private view** — you can only see claims from your team ({active_tl}).")

    # ── Load raw claims ───────────────────────────────────────────
    raw_claims = load_claims()                      # your existing loader

    # Apply privacy filter
    visible_claims = filter_claims_for_user(raw_claims, active_tl)
    ...
    with tab_log:
        with st.container(border=True):
            ...
            with col_left:
                # Engineer selector — only from active_tl's team
                my_engineers = [e for t in ALL_TEAMS
                                if t["tl"] == active_tl
                                for e in t["engineers"]]
                engineer = st.selectbox("Engineer", my_engineers,
            ...
            with col_right:
                ...
                # Category auto-fills from claim type
                category_map = {
                    "Visit delay":       "VISIT",
        ...
        if st.button("➕ Submit claim", type="primary", use_container_width=True):
            new_claim = {
                ...
                "team_leader":  active_tl,          # ← privacy tag
                "engineer":     engineer,
                "type":         claim_type,
                "category":     category,
                "date":         str(claim_date),
            ...
            save_claim(new_claim)                   # your existing saver
    ...
    with tab_summary:
        if not visible_claims:
            ...
            # If admin, show a team-breakdown selector
            if is_admin:
                all_tls = sorted(df["team_leader"].dropna().unique().tolist())
                selected_tls = st.multiselect(
                    "Filter by team leader", all_tls, default=all_tls
                )
            ...
            c2.metric("Engineers affected",
                      df["engineer"].nunique() if "engineer" in df else 0)
            c3.metric("This month",
                      len(df[df["date"].str.startswith(
                          datetime.now().strftime("%Y-%m"))]) if "date" in df else 0)
    ...
    with tab_history:
        if not visible_claims:
            ...
            # Column visibility — hide team_leader column for non-admins
