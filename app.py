...
def _norm_attachments(c):
    """Return a list of {data, name} dicts. Handles both the new
    'attachments' list and the legacy single 'attachment'/'attachment_name'."""
    out = []
    lst = c.get("attachments")
    if isinstance(lst, list):
        for a in lst:
            if isinstance(a, dict) and a.get("data"):
                out.append({"data": a["data"], "name": a.get("name", "attachment")})
    # legacy single attachment (only add if not already represented)
    if c.get("attachment"):
        out.append({"data": c["attachment"],
                    "name": c.get("attachment_name", "attachment")})
    return out
...
def _norm(c):
    return {
        ...
        "attachments":     _norm_attachments(c),
...
# ══════════════════════════════════════════════════════════════════
    ...
    with tl:
        _msg = st.session_state.pop("claim_saved_msg", None)
        if _msg:
            st.success(_msg)
        ...
        ufs  = st.file_uploader("📎 Attach files (image/PDF, max 2 MB each)",
                                 type=["jpg","jpeg","png","pdf"],
                                 accept_multiple_files=True, key="att_up")
        attachments = []
        if ufs:
            for uf in ufs:
                if uf.size > 2*1024*1024:
                    st.warning(f"⚠️ {uf.name} too large (>2 MB) — skipped")
                    continue
                attachments.append({"data": base64.b64encode(uf.read()).decode(),
                                    "name": uf.name})
                st.success(f"📎 {uf.name}")
        if st.button("➕ Submit claim", type="primary", use_container_width=True):
            first = attachments[0] if attachments else {"data": "", "name": ""}
            save_claim({"id":str(int(datetime.now().timestamp())),"team_leader":active_tl,
                        "engineer":eng,"type":ctyp,"category":cat,"date":str(cdt),
                        "description":desc,
                        "attachment":first["data"],"attachment_name":first["name"],
                        "attachments":attachments,
                        "logged_at":datetime.now().isoformat()})
            st.session_state["claim_saved_msg"] = f"✅ Claim logged for {eng}."
            st.rerun()
    ...
    with th:
        if not visible: st.info("No history yet.")
            ...
            atts = [c for c in visible if _norm_attachments(c)]
            if atts:
                st.markdown("---")
                st.markdown("**📎 Attachments**")
                for c in atts:
                    files = _norm_attachments(c)
                    if not files: continue
                    label = f"📎 {c.get('engineer','')} — {c.get('date','')} — {c.get('type','')} ({len(files)} file{'s' if len(files)>1 else ''})"
                    with st.expander(label):
                        for a in files:
                            ab64 = a.get("data",""); an = a.get("name","attachment")
                            if not ab64: continue
                            try:
                                bd = base64.b64decode(ab64)
                                if an.lower().endswith((".jpg",".jpeg",".png")):
                                    st.image(bd, caption=an)
                                else:
                                    st.markdown(f'<a href="data:application/pdf;base64,{ab64}" download="{an}">⬇️ Download {an}</a>',unsafe_allow_html=True)
                            except: st.warning(f"Could not display {an}.")
