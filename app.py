...
# Match by EMAIL (not name - checked-in rows have empty EngineerName)
# Build email->canonical name map from my team
my_email_to_name = {}
for t in ALL_TEAMS:
    for eng in t["engineers"]:
        # generate email from name
        email = eng.lower().replace(" ", ".") + "@socotec.com"
        my_email_to_name[email] = eng
# Override with known exact emails for Riyadh team
KNOWN_EMAILS = {
...
my_email_to_name.update(KNOWN_EMAILS)
...
if not checked_in_df.empty and "EngineerEmail" in checked_in_df.columns:
    ...
    for _, r in checked_in_df.iterrows():
        ...
        # Only process rows for today AND that are genuine check-ins (not "No Response")
        if row_date != today_str: continue
        if "No Response" in status or "no response" in status.lower(): continue
        if not email: continue
        # Find canonical name
        matched = my_email_to_name.get(email)
        if not matched:
            # try to find by partial match in my_engineers
            for eng in my_engineers:
                if eng.lower().replace(" ",".") in email:
                    matched = eng
                    break
        if matched and matched in my_engineers:
            checked_names.add(matched)
            visits_val = str(r.get("No_x002e__of_visits ", r.get("No_of_visits", "0"))).strip()
            att_rows.append({
                "Engineer": matched,
                "Status":   "in",
                "Check-in": str(r.get("CheckInTime",""))[:5] if r.get("CheckInTime") else "—",
                "Location": str(r.get("WorkLocation","")) or "—",
                "Visits":   visits_val or "0",
            })
